...
import dong.lan.shundai.ui.AddFriendActivity;
import dong.lan.shundai.ui.NewFriendActivity;
import dong.lan.shundai.ui.RecentActivity;
...
public class MyMessageReceiver extends BroadcastReceiver {
				...
				if(fromId!=null && !BmobDB.create(context,toId).isBlackUser(fromId)){//该消息发送方不为黑名单用户
					if(TextUtils.isEmpty(tag)){//不携带tag标签--此可接收陌生人的消息
						...
						if(tag.equals(BmobConfig.TAG_ADD_CONTACT)){
							...
							//显示通知
							showOtherNotify(context, username, toId,  username+"同意添加您为好友", AddFriendActivity.class);
						...
						}else if(tag.equals(MyConstant.UPDATE_USER_INFO))
						{
							System.out.println("收到远方的推送哈哈");
						}
					}
	...
	public void showOtherNotify(Context context,String username,String toId,String ticker,Class<?> cls){
		...
		if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
			//同时提醒通知
			BmobNotifyManager.getInstance(context).showNotify(isAllowVoice,isAllowVibrate,R.drawable.default_head, ticker,username, ticker.toString(),NewFriendActivity.class);
		}
	}
